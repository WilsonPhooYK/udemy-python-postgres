import os
import psycopg2
from psycopg2.errors import DivisionByZero # type: ignore
...
def prompt_create_poll(connection: Connection):
...
def list_open_polls(connection: Connection):
...
def prompt_vote_poll(connection: Connection):
...
def show_poll_votes(connection: Connection):
    ...
    try:
        ...
        for _, option_text, count, percentage in poll_and_votes:
            print(f"{option_text} got {count} votes ({percentage:.2f}% of total)")


def randomize_poll_winner(connection: Connection):
...
def menu():
    ...
    connection: Connection = psycopg2.connect(database_uri) # type: ignore
